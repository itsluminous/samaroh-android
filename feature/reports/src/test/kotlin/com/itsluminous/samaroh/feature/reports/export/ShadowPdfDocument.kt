...
/**
 * Robolectric ships no natives for `android.graphics.pdf`, so [PdfDocument] cannot run
 * as-is in unit tests. This test-only shadow keeps the export test meaningful: every
 * page hands the renderer a REAL bitmap-backed [Canvas], so the full table painter
 * (title, header repetition, pagination, footer) executes against real Skia; only the
 * final serialization is replaced with a minimal `%PDF` byte stream whose size scales
 * with the finished page count. (Same approach as the invoice renderer's test shadow.)
 */
